...
public class BigFraction {
   ...
   * (int num, int denom), or (String str). In case of str, it has to be in the format
   * "a/b" where a, b ∈ ℤ; when correctly initialized, the constructor will declare num
   * = a, denom = b. 
   * @comments This class, when called, initializes the num and denom variable as stated.
  ...
  public BigFraction(String str) throws IllegalArgumentException{
    ...
    if(tempVar.length < 2){
      try{
        this.num = BigInteger.valueOf(Integer.parseInt(tempVar[0].trim())); // edge case: when not enough input was given.
        return;
      }catch(Exception e){
        throw new IllegalArgumentException("Invalid input!");
      }
   ...
   * @comments The edge cases are not specified as an if statement as the constructors do
   ...
   * @comments Becareful when flipping a 0 variable. Also, this function is used as a helper
   ...
   * @comments It will return 0 when the fraction is of the form "0/a." Also, it will return 
   * NaN when there was a interaction that made the variable become "0/0." In addition, the 
